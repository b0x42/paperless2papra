...
async function migrateOneDocument(
  ...
  try {
    ...
    for (const tagId of papraTagIds) {
      await ctx.client.forOrganization(ctx.orgId).addTagToDocument({ documentId, tagId })
    }
  }
  catch (err) {
...
export async function migrate(
  ...
  const ctx: MigrationContext = { paperlessUrl, paperlessToken, papraUrl, papraToken, orgId, client, tagMap, correspondentMap, docTypeMap }
