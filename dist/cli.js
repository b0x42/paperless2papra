...
function resolveNextUrl(baseUrl, next) {
	...
	if (parsed.hostname !== base.hostname) throw new TypeError(`Pagination URL origin mismatch: expected ${base.hostname}, got ${parsed.hostname}`);
...
async function patchDocument(ctx, docId, body) {
	await ofetch(`${ctx.papraUrl}/api/organizations/${ctx.orgId}/documents/${docId}`, {
		method: "PATCH",
		headers: {
			"Authorization": `Bearer ${ctx.papraToken}`,
...
async function migrate(data, paperlessUrl, paperlessToken, papraUrl, papraToken, orgId) {
	...
	for (let i = 0; i < data.documents.length; i++) {
		...
		try {
			const status = await migrateOneDocument(doc, i, data.documents.length, ctx);
...
function requireArgs(args, keys) {
	...
	if (missing.length > 0) throw new Error(`Missing required option(s): ${missing.map((k) => `--${k}`).join(", ")}`);
}
...
async function preflightPapra(url, token, orgId) {
	...
	if (missing.length > 0) throw new Error(`Papra API key missing permissions: ${missing.join(", ")}`);
	const { organizations } = await client.listOrganizations();
	if (!organizations.some((o) => o.id === orgId)) throw new Error(`Papra organization "${orgId}" not found`);
}
